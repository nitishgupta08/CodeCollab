...
function SpaceTabs({topTabsData, value, dispatch, spaceData}) {
    ...
    const closeOpenTab = (id) => {
        ...
        dispatch({type:'updateEditorLanguage',payload:i === 0 ? topTabsData[i].lang: topTabsData[i-1].lang}); // Error if i-1 < 0
        const si = spaceData.findIndex(x => x.id === (i === 0 ? topTabsData[i].id: topTabsData[i-1].id)); // Error if i-1 < 0
            ...
            {topTabsData && topTabsData.length > 0 && (
                ...
                >
                    {topTabsData.map((item, id) => {
                        return <OpenStyledTab disableRipple label={
                            <Box component="span" sx={{ display: "flex", alignItems: "center" }}>
                                {item.fileName}
                                <Box>
                                    <IconButton component="span" sx={{ ml: 1 }} onClick={() => closeOpenTab(item.id)}>
